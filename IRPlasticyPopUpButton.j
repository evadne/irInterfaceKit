...
//	Evadne Wu at Iridia, 2010
	...
	kIRPlasticyDefaultPopUpButtonStyle = @"IRPlasticyPopUpButton";
	
	
	
	
	
@implementation IRPlasticyPopUpButton : CPPopUpButton {
	
	IRPlasticyPopUpButtonStyle style;
...
+ (CGInset) insetsForStyle:(IRPlasticyPopUpButtonStyle)style {
	
	switch (style) {
		
		case kIRPlasticyDefaultPopUpButtonStyle:
		return {
				
			bezelColor: CGInsetMake(0, 32, 0, 16),
			bezel: CGInsetMake(-4, -4, -4, -4),
			content: CGInsetMake(0, 24, 0, 16),
...
+ (id) buttonWithTitle:(CPString)inTitle {
	...
	return [self buttonWithTitle:inTitle style:kIRPlasticyDefaultPopUpButtonStyle];
